...
export function ScanerPage() {
  ...
  // Fetch products from API for the current page
  useEffect(() => {
    const fetchProducts = async () => {
      try {
        setLoading(true);
        const response = await productApi.getAll(page);
        const { data, totalPages } = response.data; // Adjust based on API response structure
        setProducts(data);
        setTotalPages(totalPages || 1); // Update total pages if provided by API
      } catch (error) {
        toast({
          title: 'Error',
    ...
    <div className="space-y-6">
      ...
      <div className="grid gap-6 md:grid-cols-2">
        <div className="flex flex-col space-y-6">
          <Card>
            <CardHeader>
              <CardTitle className=" callable items-center">
                <ShoppingBag className="mr-2 h-5 w-5" />
                Scan Products
              </CardTitle>
              <CardDescription>
